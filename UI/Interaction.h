...
public:
	...
	Textbox(const std::string &_text, const int &char_size, const sf::Color &char_color,
		const sf::Vector2f &position, const sf::Vector2f &box_size, const sf::Color &box_color, const bool &break_line = false) {
		font_tmp.loadFromFile(CONST::PATH::DEFAULT_FONT);
	...
	std::string fix_text(std::string _text);
	...
	void align_top_left(const float &offset_left = 5, const float &offset_up = 5);
...
public:
	...
	bool idle = true;
	float align_offset_left = 5;
	float align_offset_top = 5;
	...
	void set_align_offset_top(const float &offset);

	void set_break_line(const bool &_break_line = true);
...
public:
	...
	Button_Textbox(const Textbox &_textbox, const sf::Color &hover_outline_color,
		const float &hover_outline_thickness = -2) {
		textbox = _textbox;
...
public:
	...
	Button_Sprite(const sf::Texture &_texture, const sf::Vector2f &position, const sf::Vector2f &size,
		...
		idle_sprite.setScale(size.x / idle_sprite.getGlobalBounds().width,
			size.y / idle_sprite.getGlobalBounds().height);
...
class Interaction {
public:
	std::vector<Input_Textbox *> list_inptb;
	std::vector<Button_List<Button_Textbox>> type1_buttons;
	std::vector<Button_List<Button_Sprite>> type2_buttons;
