...
private:
    ...
    AVLNode *root_;
    size_type size_;
    std::function<bool(const value_type &, const value_type &)> comp_;
    ...
    AVLNode *erase(const value_type &data, AVLNode *node) {
        if (!node) return nullptr;

        // Searching for the element
        if (comp_(data, node->data_))
            node->left_ = erase(data, node->left_);
        else if (comp_(node->data_, data))
            node->right_ = erase(data, node->right_);
            // Element found
        else {
            AVLNode *temp;
            ...
            if (node->left_ && node->right_) {
                ...
                node->right_ = erase(node->data_, node->right_);
            }
...
public:
    ...
    explicit AVLTree(const std::function<bool(const value_type &, const value_type &)> &comp)
